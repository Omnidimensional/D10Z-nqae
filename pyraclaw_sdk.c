/*
 * ══════════════════════════════════════════════════════════════════════════════
 * Pyraclaw-TTA EMBEDDED SDK - IMPLEMENTACIÓN
 * ══════════════════════════════════════════════════════════════════════════════
 */
...
struct pyraclaw_node {
    char node_id[Pyraclaw_MAX_NODE_ID_LEN];
    pyraclaw_state_t state;
    ...
    pyraclaw_callbacks_t callbacks;
    bool initialized;
};
...
static uint32_t vector3_magnitude(const pyraclaw_vector3_t *v) {
    int64_t sum = (int64_t)v->x * v->x + (int64_t)v->y * v->y + (int64_t)v->z * v->z;
    return isqrt((uint32_t)(sum / 1000000));
}

static void prune_inactive_neighbors(pyraclaw_node_t *node, uint32_t current_time_ms) {
...
static void propagate_coherence(pyraclaw_node_t *node) {
...
static void compute_local_e_tta(pyraclaw_node_t *node) {
...
static int find_neighbor(pyraclaw_node_t *node, const char *node_id) {
    for (int i = 0; i < node->neighbor_count; i++) {
...
static const char* find_best_next_hop(pyraclaw_node_t *node, const char *dest_id) {
    int best_idx = -1;
...
void pyraclaw_config_default(pyraclaw_config_t *config) {
    memset(config, 0, sizeof(*config));
    strcpy(config->node_id, "Pyraclaw-NODE");
    config->initial_phi = 500;
    config->alpha_decay = 50;
    config->beta_coupling = 200;
    config->heartbeat_interval_ms = Pyraclaw_HEARTBEAT_INTERVAL_MS;
    config->neighbor_timeout_ms = Pyraclaw_NEIGHBOR_TIMEOUT_MS;
}

pyraclaw_node_t* pyraclaw_node_create(const pyraclaw_config_t *config) {
    if (!config) return NULL;
    
    pyraclaw_node_t *node = (pyraclaw_node_t*)malloc(sizeof(pyraclaw_node_t));
    if (!node) return NULL;
    
    memset(node, 0, sizeof(*node));
    strncpy(node->node_id, config->node_id, Pyraclaw_MAX_NODE_ID_LEN - 1);
...
void pyraclaw_node_destroy(pyraclaw_node_t *node) {
    if (node) free(node);
}

pyraclaw_error_t pyraclaw_node_tick(pyraclaw_node_t *node, uint32_t current_time_ms) {
    if (!node || !node->initialized) return Pyraclaw_ERROR_NOT_INITIALIZED;
    ...
    if (current_time_ms - node->last_heartbeat_ms >= node->heartbeat_interval_ms) {
        ...
        if (node->callbacks.send_heartbeat) {
            ...
            strncpy(hb.node_id, node->node_id, Pyraclaw_MAX_NODE_ID_LEN - 1);
...
pyraclaw_error_t pyraclaw_node_update_state(pyraclaw_node_t *node, const pyraclaw_state_t *state) {
    if (!node || !state) return Pyraclaw_ERROR_INVALID_PARAM;
    
    pyraclaw_level_t old_level = pyraclaw_phi_to_level(node->state.phi);
    node->state = *state;
    pyraclaw_level_t new_level = pyraclaw_phi_to_level(node->state.phi);
...
pyraclaw_error_t pyraclaw_node_update_position(pyraclaw_node_t *node, int32_t lat_e6, int32_t lon_e6, int32_t alt_m) {
    if (!node) return Pyraclaw_ERROR_INVALID_PARAM;
...
pyraclaw_error_t pyraclaw_node_process_heartbeat(
    pyraclaw_node_t *node,
    const pyraclaw_heartbeat_t *heartbeat,
    uint16_t distance_cm,
    int8_t signal_strength,
    pyraclaw_conn_type_t conn_type
) {
    if (!node || !heartbeat) return Pyraclaw_ERROR_INVALID_PARAM;
    if (strcmp(heartbeat->node_id, node->node_id) == 0) return Pyraclaw_OK; /* Ignorar propio */
    
    int idx = find_neighbor(node, heartbeat->node_id);
    
    if (idx < 0) {
        if (node->neighbor_count >= Pyraclaw_MAX_NEIGHBORS) {
    ...
    pyraclaw_neighbor_t *neighbor = &node->neighbors[idx];
    strncpy(neighbor->node_id, heartbeat->node_id, Pyraclaw_MAX_NODE_ID_LEN - 1);
...
pyraclaw_error_t pyraclaw_node_send(
    pyraclaw_node_t *node,
    const char *dest_id,
    const uint8_t *payload,
    uint16_t payload_len
) {
    if (!node || !dest_id || !payload || payload_len > Pyraclaw_MAX_PACKET_SIZE) {
        return Pyraclaw_ERROR_INVALID_PARAM;
    }
    
    pyraclaw_packet_t packet;
    ...
    strncpy(packet.source_id, node->node_id, Pyraclaw_MAX_NODE_ID_LEN - 1);
    strncpy(packet.dest_id, dest_id, Pyraclaw_MAX_NODE_ID_LEN - 1);
    packet.hop_count = 0;
    ...
    if (next_hop) {
        ...
        return Pyraclaw_ERROR_NEIGHBOR_NOT_FOUND;
    }
}

pyraclaw_error_t pyraclaw_node_receive(pyraclaw_node_t *node, const pyraclaw_packet_t *packet) {
    if (!node || !packet) return Pyraclaw_ERROR_INVALID_PARAM;
    ...
    /* Reenviar */
    if (packet->hop_count >= Pyraclaw_MAX_HOP_COUNT) {
        node->packets_dropped++;
        return Pyraclaw_ERROR_TIMEOUT;
    }
    
    const char *next_hop = find_best_next_hop(node, packet->dest_id);
    
    if (next_hop) {
        pyraclaw_packet_t fwd_packet = *packet;
...
pyraclaw_error_t pyraclaw_node_get_state(const pyraclaw_node_t *node, pyraclaw_state_t *state) {
    if (!node || !state) return Pyraclaw_ERROR_INVALID_PARAM;
    *state = node->state;
    return Pyraclaw_OK;
}

pyraclaw_error_t pyraclaw_node_get_metrics(const pyraclaw_node_t *node, pyraclaw_metrics_t *metrics) {
    if (!node || !metrics) return Pyraclaw_ERROR_INVALID_PARAM;
    
    metrics->phi = node->state.phi;
    metrics->level = pyraclaw_phi_to_level(node->state.phi);
...
pyraclaw_error_t pyraclaw_node_get_neighbors(
    const pyraclaw_node_t *node,
    pyraclaw_neighbor_t *neighbors,
    uint8_t *count,
    uint8_t max_count
) {
    if (!node || !neighbors || !count) return Pyraclaw_ERROR_INVALID_PARAM;
    
    uint8_t n = (node->neighbor_count < max_count) ? node->neighbor_count : max_count;
    memcpy(neighbors, node->neighbors, n * sizeof(pyraclaw_neighbor_t));
    *count = n;
    
    return Pyraclaw_OK;
}

pyraclaw_level_t pyraclaw_node_get_level(const pyraclaw_node_t *node) {
    if (!node) return Pyraclaw_LEVEL_ISOLATED;
    return pyraclaw_phi_to_level(node->state.phi);
}

uint32_t pyraclaw_node_get_e_tta(const pyraclaw_node_t *node) {
...
pyraclaw_level_t pyraclaw_phi_to_level(uint16_t phi) {
    if (phi >= 900) return Pyraclaw_LEVEL_OPTIMAL;
    if (phi >= 700) return Pyraclaw_LEVEL_OPERATIONAL;
    if (phi >= 500) return Pyraclaw_LEVEL_DEGRADED;
    if (phi >= 300) return Pyraclaw_LEVEL_CRITICAL;
    return Pyraclaw_LEVEL_ISOLATED;
}

uint32_t pyraclaw_vector3_distance(const pyraclaw_vector3_t *a, const pyraclaw_vector3_t *b) {
    pyraclaw_vector3_t diff = { a->x - b->x, a->y - b->y, a->z - b->z };
    return vector3_magnitude(&diff);
}

uint16_t pyraclaw_heartbeat_serialize(const pyraclaw_heartbeat_t *hb, uint8_t *buf, uint16_t len) {
    if (len < 80) return 0;
    ...
    memcpy(&buf[pos], hb->node_id, Pyraclaw_MAX_NODE_ID_LEN); pos += Pyraclaw_MAX_NODE_ID_LEN;
...
pyraclaw_error_t pyraclaw_heartbeat_deserialize(const uint8_t *buf, uint16_t len, pyraclaw_heartbeat_t *hb) {
    ...
    memcpy(hb->node_id, &buf[pos], Pyraclaw_MAX_NODE_ID_LEN); pos += Pyraclaw_MAX_NODE_ID_LEN;
...
uint16_t pyraclaw_rssi_to_distance_cm(int8_t rssi, int8_t tx_power) {
...
const char* pyraclaw_level_to_string(pyraclaw_level_t level) {
    switch (level) {
        case Pyraclaw_LEVEL_OPTIMAL: return "OPTIMAL";
        case Pyraclaw_LEVEL_OPERATIONAL: return "OPERATIONAL";
        case Pyraclaw_LEVEL_DEGRADED: return "DEGRADED";
        case Pyraclaw_LEVEL_CRITICAL: return "CRITICAL";
        case Pyraclaw_LEVEL_ISOLATED: return "ISOLATED";
        default: return "UNKNOWN";
    }
}

const char* pyraclaw_error_to_string(pyraclaw_error_t error) {
    switch (error) {
        case Pyraclaw_OK: return "OK";
        case Pyraclaw_ERROR_INVALID_PARAM: return "INVALID_PARAM";
        case Pyraclaw_ERROR_NO_MEMORY: return "NO_MEMORY";
        case Pyraclaw_ERROR_BUFFER_FULL: return "BUFFER_FULL";
        case Pyraclaw_ERROR_NOT_INITIALIZED: return "NOT_INITIALIZED";
        case Pyraclaw_ERROR_NEIGHBOR_NOT_FOUND: return "NEIGHBOR_NOT_FOUND";
        case Pyraclaw_ERROR_TIMEOUT: return "TIMEOUT";
        case Pyraclaw_ERROR_NETWORK: return "NETWORK";
        default: return "UNKNOWN";
    }
}
