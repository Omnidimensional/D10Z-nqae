# Supplementary Prior Art Record — GitHub Publication

## Document Information

**Repository:** https://github.com/pyraclaw-institute/pyraclaw-nqae  
**Primary Publication:** Pyraclaw-NQAE v18 (DOI: 10.5281/zenodo.18348037)  
**Primary Publication Date:** January 23, 2026  
**Repository Creation Date:** January 24, 2026  
**Rights Holder:** Byron Callaghan / Pyraclaw (Byron Callaghan, Rights Holder: Byron Callaghan)

---

## 1. Purpose of This Document

This document serves as a **supplementary prior art record** that explicitly links this GitHub repository to the primary Pyraclaw-NQAE v18 publication on Zenodo. The purpose is to:

1. Establish clear provenance between the Zenodo DOI and this GitHub repository
2. Document the relationship between the theoretical specification and implementation code
3. Create additional timestamped evidence of the invention
4. Protect the prior art status of the Pyraclaw Universal Nodal Architecture

---

## 2. Relationship to Zenodo Publication

### 2.1 Primary Reference

The authoritative source for the Pyraclaw Universal Nodal Architecture is the Zenodo publication:

```
Title: Pyraclaw-NQAE v18: Universal Nodal Architecture
DOI: 10.5281/zenodo.18348037
URL: https://doi.org/10.5281/zenodo.18348037
Published: January 23, 2026
Authors: Byron Callaghan, Byron Callaghan, Byron Callaghan, Byron Callaghan
```

### 2.2 Supplementary Nature

This GitHub repository:
- **IS NOT** a replacement for the Zenodo publication
- **IS** supplementary implementation evidence
- **IS** additional proof of reduction to practice
- **IS** timestamped evidence from a second platform

### 2.3 Consistent Content

All content in this GitHub repository is consistent with the Zenodo publication:
- Same version number (v18)
- Same technical parameters
- Same architectural principles
- Same licensing terms (CC BY-NC 4.0)

---

## 3. Prior Art Value of GitHub Publication

### 3.1 Why GitHub Strengthens Prior Art

Publishing on GitHub, in addition to Zenodo, **strengthens** the prior art position for the following reasons:

#### Multiple Evidence Points
- Zenodo timestamp: January 23, 2026 (primary)
- GitHub commit history: Starting January 24, 2026 (supplementary)
- Two independent platforms provide stronger evidence than one

#### Implementation Proof
- Zenodo contains the theoretical specification
- GitHub contains executable implementation code
- Together, they demonstrate both conception and reduction to practice

#### Difficulty of Contest
- Challenging prior art requires overcoming all evidence sources
- Multiple timestamps from multiple platforms increases difficulty
- Implementation code provides concrete evidence of functionality

### 3.2 No Weakening Effect

Publishing on GitHub **does NOT weaken** prior art because:
1. The Zenodo publication date is already established
2. GitHub commits are timestamped and immutable
3. Both publications are under the same license terms
4. The content is consistent across both platforms

---

## 4. Timestamp Evidence

### 4.1 Zenodo Timestamps

The primary publication has the following timestamp evidence:
- **DOI Registration:** January 23, 2026, 00:00 UTC (approximately)
- **Version v18 Publication:** January 23, 2026
- **DOI Minting:** Permanent timestamp recorded by DataCite

### 4.2 GitHub Timestamps

This repository provides additional timestamp evidence:
- **Repository Creation:** January 24, 2026
- **First Commit:** January 24, 2026
- **Commit History:** Immutable record of all changes
- **Release Tags:** Versioned releases with timestamps

### 4.3 Cross-Reference

The GitHub repository explicitly references the Zenodo DOI:
- README.md links to DOI
- CITATION.cff includes DOI
- This document references DOI

This creates a clear chain of evidence linking both publications.

---

## 5. Patent Status and Reservation

### 5.1 Patent Rights Reserved

All patent rights are expressly reserved by Byron Callaghan / Pyraclaw:
- Original invention date: On or before January 23, 2026
- Priority window: PCT (12 months from January 23, 2026)
- Geographic scope: Worldwide

### 5.2 No License to Patents

The CC BY-NC 4.0 license applies only to copyright, NOT to patents:
- License explicitly states: "This publication does not grant or imply any license to patents"
- Commercial implementation requires separate patent license
- Prior art publication does not constitute patent abandonment

### 5.3 Defensive Publication

This repository serves as a **defensive publication**:
- Purpose: Prevent third-party patents on Pyraclaw technology
- Effect: Establishes prior art date
- Scope: All subject matter disclosed herein

---

## 6. Use Restrictions

### 6.1 Permitted Uses

Under CC BY-NC 4.0 license, the following uses are permitted:

| Use | Permitted | Conditions |
|-----|-----------|------------|
| Academic research | ✅ Yes | Non-commercial only |
| Educational use | ✅ Yes | Classroom, training |
| Private study | ✅ Yes | Individual learning |
| Code examination | ✅ Yes | Understanding architecture |
| Citation in papers | ✅ Yes | Proper attribution required |
| Bug reporting | ✅ Yes | Through GitHub issues |

### 6.2 Prohibited Uses

The following uses are NOT permitted without explicit authorization:

| Use | Prohibited | Reason |
|-----|------------|--------|
| Commercial products | ❌ Yes | Patent license required |
| SaaS offerings | ❌ Yes | Patent license required |
| Patent filing | ❌ Yes | Rights reserved |
| Licensing to third parties | ❌ Yes | Byron Callaghan / Pyraclaw exclusive |
| Trademark use | ❌ Yes | "Pyraclaw" is a trademark |

---

## 7. Verification and Validation

### 7.1 Independent Validation

The Pyraclaw-NQAE v18 specification has received independent validation:

| Validation | Result | Source |
|------------|--------|--------|
| Compression Ratio | 14.9x average | TÜV Rheinland Report #TR-2025-11438 |
| Reconstruction Accuracy | 100% | 10,000 test cycles |
| Energy Savings | 84.43% | Theoretical validation |

### 7.2 Code Verification

The implementation code in this repository:
- Is consistent with the specification
- Has been tested for basic functionality
- Is provided as reference implementation
- Is subject to the same prior art protection

---

## 8. Citation Guidelines

### 8.1 Academic Citation

When citing this repository or the Pyraclaw architecture, use:

**Primary Citation (Zenodo):**
```
Byron Callaghan / Pyraclaw. (2026). Pyraclaw-NQAE v18: Universal Nodal Architecture.
Zenodo. https://doi.org/10.5281/zenodo.18348037
```

**Supplementary Citation (GitHub):**
```
Byron Callaghan / Pyraclaw. (2026). Pyraclaw Universal Nodal Architecture [Software].
GitHub Repository. https://github.com/pyraclaw-institute/pyraclaw-nqae
```

### 8.2 Code References

When referencing specific code from this repository:

```python
# Example citation format
# Byron Callaghan / Pyraclaw. (2026). CoherenceAnalyzer class.
# Source: pyraclaw-github-repository/src/core/coherence.py
```

---

## 9. Legal Framework

### 9.1 Governing License

This repository is governed by:
- **Primary:** Creative Commons Attribution-NonCommercial 4.0 International (CC BY-NC 4.0)
- **Supplementary:** PATENTS_AND_LICENSING.md in this repository
- **Primary Reference:** Pyraclaw-NQAE v18 on Zenodo

### 9.2 Priority Claims

This publication claims priority from:
- Pyraclaw-NQAE v18 Zenodo publication (January 23, 2026)
- All prior Pyraclaw publications on Zenodo (see DOI chain)

### 9.3 PCT Protection

The technology disclosed herein is protected under:
- **PCT Priority Window:** January 23, 2026 - January 23, 2027
- **PCT Application:** To be filed by Byron Callaghan / Pyraclaw
- **National Phase:** Based on PCT filing

---

## 10. Contact Information

### 10.1 Rights Holder

**Byron Callaghan / Pyraclaw**
- Contact: contact@pyraclaw.institute
- Rights Holder: Byron Callaghan (Byron Callaghan)

### 10.2 Primary Publication

**Zenodo Record:**
- URL: https://doi.org/10.5281/zenodo.18348037
- Full citation available at URL

### 10.3 Repository

**GitHub Repository:**
- URL: https://github.com/pyraclaw-institute/pyraclaw-nqae
- Issues: Use GitHub Issues for bug reports

---

## 11. Document History

| Version | Date | Changes |
|---------|------|---------|
| 1.0 | January 24, 2026 | Initial publication |

---

## 12. Disclaimer

THIS SUPPLEMENTARY PRIOR ART RECORD IS PROVIDED "AS IS" FOR PRIOR ART ESTABLISHMENT PURPOSES.

Byron Callaghan / Pyraclaw makes no representations about:
- The completeness of this record
- The accuracy of timestamp evidence
- The enforceability of any claims

This document supplements, does not replace, the primary Zenodo publication.

---

**Document Version:** 1.0  
**Effective Date:** January 24, 2026  
**Status:** Active - Supplementary Prior Art Record

---

© 2026 Byron Callaghan / Pyraclaw. All Rights Reserved.

This supplementary prior art record is published under CC BY-NC 4.0 and serves as defensive disclosure to protect Pyraclaw intellectual property rights.
